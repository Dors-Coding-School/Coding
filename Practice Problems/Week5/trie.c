...
// Check if word is in trie, return true if found, false otherwise
bool check(char* word)
{
    ...
    {
        // Convert the letter to lowercase and get its corresponding index
        int index = tolower(word[i]) - 'a';

        // If the index is out of bounds, the word is not in the trie
        if (index < 0 || index >= SIZE_OF_ALPHABET)
        {
            return false;
        }

        // If the child node for the letter is NULL, the word is not in the trie
        if (cursor->children[index] == NULL)
        {
            return false;
        }

        // Move to the child node for the next letter
        cursor = cursor->children[index];
    }

    // If we reached the end of the word and the final node is marked as a word, it's in the trie
